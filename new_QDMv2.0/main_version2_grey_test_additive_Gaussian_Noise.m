...
for i = 1:12
%     fprintf("variance:%.3f\n",variance);
    Img_noise = imnoise(Img_w, 'gaussian', mean, variance); % generate Gaussian noise
%     figure;imshow(Img_noise);
    ...
    [arbitration_noise,BER] = detection_my2(Bm,G,A_noise_abs,b_k,Delta,delta,flag);
    fprintf("%.1f,",BER);
%     if arbitration_noise == 1
%         disp("图像添加加性高斯噪声后水印提取成功")
%     else
%         disp("图像添加加性高斯噪声后水印提取失败")
%     end
