...
img = imread("10 plant512_rgb.jpg");
% 若是彩色图像则转为灰度图像
...
fprintf("BER:");
for i = 1:10
%     fprintf("compressionRatio:%.1f\n",compressionRatio);
    imwrite(Img_w,'img_jpeg2000.jp2','CompressionRatio',compressionRatio);
    img_jpeg2 = imread('img_jpeg2000.jp2');
%     figure;imshow(img_jpeg2);
    img_jpeg2 = double(img_jpeg2);
    [A_jpeg2]=zernike_A(img_jpeg2, V, PQ);
    A_jpeg2_abs = abs(T_multiply*A_jpeg2/A_jpeg2(1));
    [arbitration_jpeg2,BER] = detection_my2(Bm,G,A_jpeg2_abs,b_k,Delta,delta,flag);
    fprintf("%.1f,",BER);
%     if arbitration_jpeg2 == 1
%         disp("图像JPEG2000压缩后水印提取成功")
%     else
%         disp("图像JPEG2000压缩后水印提取失败")
%     end
