...
center_width = 256;
center_height = 256;
imgw_size = size(Img_w);
cropped_img = clip(Img_w,cropped_width, cropped_height, center_width, center_height);
restored_Img = padding(imgw_size(1),imgw_size(2),center_height,center_width,cropped_img);
restored_img = double(restored_Img);
[A_clip] = zernike_A(restored_img, V, PQ);
A_clip_abs = abs(T_multiply*A_clip/A_clip(1));
[arbitration_clip] = detection_my2(Bm,G,A_clip_abs,b_k,Delta,delta,flag);
%[arbitration] = detection_my1(Bm,G,w_A_abs,b_k,Delta,delta,flag);
if arbitration_clip == 1
    disp("图像裁剪后水印提取成功")
else
    disp("图像裁剪后水印提取失败")
end