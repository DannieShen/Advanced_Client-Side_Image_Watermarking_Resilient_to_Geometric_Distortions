...
for i = 1:10
    fprintf("quality:%.1f\n",quality);
    imwrite(Img_w,'img_jpeg.jpg','quality',quality);
    img_jpeg = imread('img_jpeg.jpg');
%     figure;imshow(img_jpeg);
    img_jpeg = double(img_jpeg);
    [A_jpeg]=zernike_A(img_jpeg, V, PQ);
    A_jpeg_abs = abs(T_multiply*A_jpeg/A_jpeg(1));
    [arbitration_jpeg] = detection_my2(Bm,G,A_jpeg_abs,b_k,Delta,delta,flag);
    if arbitration_jpeg == 1
        disp("图像JPEG压缩后水印提取成功")
    else
        disp("图像JPEG压缩后水印提取失败")
    end
    quality = quality + 10;
end