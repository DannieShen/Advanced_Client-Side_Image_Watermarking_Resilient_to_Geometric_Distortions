...
for i = 1:10
    fprintf("angle:%d\n",ro_angle);
    I_rotation = imrotate(img_w1,ro_angle, 'bilinear', 'crop');
    figure;imshow(I_rotation);
    I_rotation = I_rotation(:,:,1);
    I_rotation = double(I_rotation);
%     crop_img = cropimg(I_rotation,img_size);
    [A_rotation]=zernike_A2(I_rotation,PQ);
