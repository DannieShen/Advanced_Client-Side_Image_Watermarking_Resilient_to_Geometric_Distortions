...
img1 = imread("10 leaf512_rgb.jpg");
% 若是彩色图像则转为灰度图像
...
img = double(img1);
% 展示图像
figure('Name','原始图像');imshow(uint8(img));
...
c_img1 = uint8(mod(round(real(c_img)),256));
figure('Name','第一次加密');imshow(c_img1);
...
% figure('Name','第二次加密');imshow(cipher1);
...
w_img1 = uint8(round(real(w_img)));
figure('Name','第一次解密');imshow(w_img1);
...
d = round(real(decryption));
difference = uint8(abs(img-d)*20);
% 计算两张图像的差异
figure('Name','difference');imshow(difference);
Img_w = uint8(round(real(decryption)));
figure;imshow(Img_w);
...
figure('Name','第二次解密');imshow(Img_w);
...
% [arbitration_noise] = detection_my1(Bm,G,SA_l,A_noise_abs,b_k, Delta, delta);
...
% [arbitration_resize] = detection_my1(Bm,G,SA_l,A_resize_abs,b_k, Delta, delta);
...
%     I_rotation = cropimg(I_rotation,img_size);
%     figure;
%     imshow(uint8(I_rotation));
...
%     [arbitration_rotation] = detection_my1(Bm,G,SA_l,A_rotation_abs,b_k, Delta, delta);
%     if arbitration_rotation == 1
%         disp("图像旋转后水印提取成功")
%     else
%         disp("图像旋转后水印提取失败")
%     end
%     ro_angle = ro_angle + 36;
% end
...
% A_rotation_abs = abs(T_multiply*A_rotation/A_rotation(1));
% [arbitration_rotation] = detection_my(Bm,G,SA_l,A_rotation_abs,b_k, Delta, delta);
% if arbitration_rotation == 1
%     disp("图像旋转后水印提取成功")
% else
%     disp("图像旋转后水印提取失败")
...
% [V_jepg,PQ]=zernike_V(img1_w, order);
% [A_jepg]=zernike_A(img1_w, V_jepg, PQ);
% A_jepg_abs = abs(T_multiply*A_jepg/A_jepg(1));
% [arbitration_jepg] = detection_my1(Bm,G,SA_l,A_jepg_abs,b_k, Delta, delta);
